...
void ledc_mod_setDutyFromISR(ledc_channel_t channel, int duty) {
    ledc_hal_set_duty_int_part(&(p_ledc_obj[LEDC_HIGH_SPEED_MODE]->ledc_hal), channel, duty);
    
    ledc_hal_set_sig_out_en(&(p_ledc_obj[LEDC_HIGH_SPEED_MODE]->ledc_hal), channel, true);
    ledc_hal_set_duty_start(&(p_ledc_obj[LEDC_HIGH_SPEED_MODE]->ledc_hal), channel, true);
    ledc_ls_channel_update(LEDC_HIGH_SPEED_MODE, channel);
}

esp_err_t ledc_set_pin_inv(int gpio_num, ledc_mode_t speed_mode, ledc_channel_t ledc_channel)
{
    LEDC_ARG_CHECK(ledc_channel < LEDC_CHANNEL_MAX, "ledc_channel");
    LEDC_ARG_CHECK(GPIO_IS_VALID_OUTPUT_GPIO(gpio_num), "gpio_num");
    LEDC_ARG_CHECK(speed_mode < LEDC_SPEED_MODE_MAX, "speed_mode");
    gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[gpio_num], PIN_FUNC_GPIO);
    gpio_set_direction(gpio_num, GPIO_MODE_OUTPUT);
    esp_rom_gpio_connect_out_signal(gpio_num, ledc_periph_signal[speed_mode].sig_out0_idx + ledc_channel, 1, 0);
    return ESP_OK;
}
